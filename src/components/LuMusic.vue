<template>
  <div class="music">
      ...
      <div @click="showList = !showList" class="bloom-container">
        <div v-show="issuspend" class="button-container-main">
          <div class="button-inner">
            <div class="back"></div>
            <div class="front">
              <svg t="1701070286445" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="21570" width="40" height="40">
                <path
                ...
                <path
                  ...
                  fill="#3D464B"
                  p-id="21575"></path>
                <path d="M249.373 624.373c0 14.65-11.92 26.57-26.57 26.57s-26.57-11.92-26.57-26.57 11.92-26.56 26.57-26.56 26.57 11.911 26.57 26.56z" fill="#B8D6C8" p-id="21576"></path>
                <path
                  ...
                  fill="#3D464B"
                  p-id="21577"></path>
                <path d="M137.973 811.423c-2.89 4.85-7.5 8.29-12.97 9.68-11.31 2.87-22.84-3.99-25.71-15.3-2.87-11.3 3.99-22.83 15.3-25.7 1.74-0.45 3.479-0.66 5.2-0.66 9.45 0 18.08 6.39 20.51 15.96 1.39 5.47 0.56 11.16-2.33 16.02z" fill="#F7A09F" p-id="21578"></path>
              </svg>
            </div>
          </div>
    ...
    <div v-show="showList" class="list-box">
      <div class="list">
        <div :class="item._id == nowMusic._id ? 'actice' : ''" @click="changeMusic(item)" v-for="(item, index) in musiclist" class="item">
...
let ended = () => {
  console.log('ended')
  if (playType.value == 0) {
    if (nowMusicIndex.value == musiclist.value.length) {
      nowMusicIndex.value = 1
    } else {
      nowMusicIndex.value += 1
    }
    nowMusic.value = musiclist.value[nowMusicIndex.value - 1]
    nextTick(() => {
      issuspend.value = false
      audio.value.play()
    })
  } else if (playType.value == 1) {
    let next = nowMusicIndex.value - 1
    while (next == nowMusicIndex.value - 1) {
...
// 上一首
let beforMusic = () => {
  if (nowMusicIndex.value == 1) {
    nowMusicIndex.value = musiclist.value.length
  } else {
    nowMusicIndex.value -= 1
  }
  nowMusic.value = musiclist.value[nowMusicIndex.value - 1]
...
let nowMusic = ref<Music>({ _id: 1, name: '', artist: '', url: '', cover: '' })
let nowMusicIndex = ref(1)
const musiclist = ref<Music[]>([
  ...
  // { id: 3, name: 'Windy Hill', artist: '羽肿', lrc: '', url: 'https://www.ytmp3.cn/down/53896.mp3', cover: 'https://imgessl.kugou.com/stdmusic/20170815/20170815070007812976.jpg' },
  // { id: 4, name: '也许是天意', artist: '赵海洋', lrc: '', url: 'https://www.ytmp3.cn/down/75787.mp3', cover: 'https://imgessl.kugou.com/uploadpic/softhead/240/20201110/20201110004118192.jpg' },
  // {
  //   id: 5,
  //   name: '瞬间的永恒',
  //   artist: '赵海洋',
  //   lrc: '',
  //   url: 'https://www.ytmp3.cn/down/53702.mp3',
  //   cover: 'https://imgessl.kugou.com/uploadpic/softhead/240/20201110/20201110004118192.jpg'
  // }, // 播放这首歌曲时的主题色},
  // { id: 6, name: '夜空的寂静', artist: '赵海洋', lrc: '', url: 'https://www.ytmp3.cn/down/75788.mp3', cover: 'https://imgessl.kugou.com/stdmusic/20170507/20170507115833669586.jpg' },
  // // { id: 7, name: '秋的思念', artist: '赵海洋', lrc: '', url: 'https://www.ytmp3.cn/down/47097.mp3', cover: 'https://imgessl.kugou.com/stdmusic/20170507/20170507115833669586.jpg' },
  // { id: 8, name: '城南花已开', artist: '三亩地', lrc: '', url: 'https://www.ytmp3.cn/down/47675.mp3', cover: 'https://imgessl.kugou.com/stdmusic/20181102/20181102115543498345.jpg' },
...
let getMusicList = async () => {
  const { data } = await $http.other.getMusicList()
  if (data.status == 0) {
    musiclist.value = data.data.data
    nowMusic.value = data.data.data[0]
  }
}
getMusicList()
</script>
